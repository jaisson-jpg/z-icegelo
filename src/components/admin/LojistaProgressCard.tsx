...
export function LojistaProgressCard({
  id,
  businessName,
  userName,
  sacosComprados,
  ...
  const porcentagem = Math.min(100, Math.floor((sacosComprados / meta) * 100));

  return (
    <div className="bg-white rounded-2xl border p-5 shadow-sm">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-bold text-[var(--zice-dark)] truncate">{businessName}</h3>
          <p className="text-xs text-gray-500">{userName}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-black text-[var(--zice-medium)]">{sacosComprados}/{meta}</p>
